...
tar czSf $resPath/oracleResources/container-scriptsFMW.tgz -C container-scripts .

# BUILD THE IMAGE (replace all environment variables)
BUILD_START=$(date '+%s')
#docker build --force-rm=$NOCACHE --no-cache=$NOCACHE $PROXY_SETTINGS -t $IMAGE_NAME -f Dockerfile . ||
echo $PROXY_SETTINGS
docker build $PROXY_SETTINGS -t $IMAGE_NAME --build-arg RESSRV="$RESSRV" -f DockerfileOpt . ||
